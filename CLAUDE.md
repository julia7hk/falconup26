...
- `backend/llm_explainer/` — structured explainer layer. `templates.py` (M8 PR1, done): pure functions turning the `risk_grade` payload into plain-English, beginner-facing explanations — reuses the engine's per-component `reason` strings verbatim so every number/ticker traces to the input; no I/O, no LLM. The LLM enrichment (prompts/, context.py, validator.py, cache.py, client.py) is PR2 (not yet implemented) and falls back to `templates.py`.
